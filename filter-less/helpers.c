...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int row = 0; row < height; row++)
    {
        ...
        {
            int avg = round((image[row][column].rgbtBlue + image[row][column].rgbtGreen + image[row][column].rgbtRed) / 3.0);
            image[row][column].rgbtBlue = image[row][column].rgbtGreen = image[row][column].rgbtRed = avg;
        }
    }
    return;
}
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    for (int row = 0; row < height; row++)
    {
        int swap = width - 1; // Initialize a variable swap here

        for (int column = 0; column < width / 2; column++)
        {
            // Swap the columns from the left with the columns from the right
            RGBTRIPLE temp = image[row][column];
            image[row][column] = image[row][swap];
            image[row][swap] = temp;
